...
export const studentNavItems: StudentNavItem[] = [
  {
    ...
    description: "Learning overview & active milestones",
  },
  {
    title: "My Courses",
    ...
    icon: BookOpen,
    badge: "2 Active",
    description: "Curriculum modules and assignments",
  },
  {
    title: "My Programs",
    ...
    icon: GraduationCap,
    badge: "1 Track",
    description: "Enrolled internship programs & tracks",
  },
  {
    title: "Certificates",
    href: "/student/certificates",
    icon: Award,
    description: "Verified graduation credentials",
  },
]

function StudentLogoMark({ className = "size-7" }: { className?: string }) {
  return (
    <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <path d="M20 4L33 12.5L20 21L7 12.5L20 4Z" fill="#182d46" />
      <path d="M7 12.5L20 21V36L7 27.5V12.5Z" fill="#101f33" />
      <path d="M33 12.5L20 21V36L33 27.5V12.5Z" fill="#213a5a" />
      <path d="M20 21L29 15L25.5 25.5L20 29V21Z" fill="#b8864d" />
      <path d="M20 21L11 15L14.5 25.5L20 29V21Z" fill="#986c3b" />
...
export function StudentSidebar({ isOpen, onClose }: StudentSidebarProps) {
  ...
  const userEmail = session?.email || "student@iconiccareer.com"
  const userInitials =
    userName
      .split(" ")
      .map((n: string) => n[0])
  ...
  const isItemActive = (href: string) => {
    if (href === "/student") {
      return pathname === "/student" || pathname === "/student/" || pathname === "/student/dashboard"
    }
  ...
  const content = (
    <div className="flex h-full flex-col bg-[#14233c] text-white border-r border-[#22385c]">
      ...
      <div className="flex h-16 items-center justify-between px-5 border-b border-[#22385c]/80 bg-[#101d33]">
        <Link to="/student" className="flex items-center gap-3 group" onClick={onClose}>
          <StudentLogoMark className="size-7.5 transition-transform group-hover:scale-105" />
          <div className="flex flex-col">
            ...
            <span className="font-mono text-[0.65rem] tracking-wider text-[#d4af7a] uppercase font-medium flex items-center gap-1">
              <Sparkles className="size-3" /> Candidate Portal
            </span>
          </div>
        </Link>
        {onClose && (
      ...
      <div className="border-t border-[#22385c] bg-[#101d33]/90 p-3 space-y-2">
        ...
        <div className="flex items-center justify-between rounded-lg bg-[#14233c] px-3 py-2.5 border border-[#22385c]">
          <div className="flex items-center gap-2.5">
            <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-[#3b82f6] to-[#60a5fa] font-semibold text-white text-xs shadow-xs">
              {userInitials}
            </div>
